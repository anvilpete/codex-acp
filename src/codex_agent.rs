use acp::schema::{
    ...
    ElicitationCapabilities, ElicitationRequestScope, ElicitationUrlCapabilities,
    ElicitationUrlMode, Implementation, InitializeRequest, InitializeResponse,
    ...
    PromptResponse, ProtocolVersion, RequestId, SessionCapabilities, SessionCloseCapabilities,
            ...
            .on_receive_request(
                {
                    ...
                    async move |request: AuthenticateRequest,
                        ...
                        let request_id = request_id_from_json(responder.id()).expect("request id must be a string or number");
                        cx.spawn(async move {
...
impl CodexAgent {
        ...
        match auth_method {
            CodexAuthMethod::ChatGpt => {
                ...
                if std::env::var("NO_BROWSER").is_ok() {
                    Self::device_code_auth(opts, self.auth_manager.clone(), cx, request_id).await?;
                } else {
    ...
    async fn device_code_auth(
        ...
        codex_login::complete_device_code_login(opts, device_code).await.map_err(Error::into_internal_error)?;
